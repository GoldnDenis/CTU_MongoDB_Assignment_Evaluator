...
public class UpdateArrayReplaceCriterion extends AssignmentCriterion {
    public UpdateArrayReplaceCriterion() {
        super(
    ...
    private void checkDocument(Document document) {
        Document pullDocument = BsonChecker.getFirstLevelOperatorDocument(document, "$pull");
        if (pullDocument != null && !pullDocument.isEmpty()) {
            currentCount++;
            satisfied = true;
        }
    }
