...
public class AggregateSkipStageCriterion extends AssignmentCriterion {
    public AggregateSkipStageCriterion(InsertedDocumentStorage mockDb) {
        super(
                mockDb,
                Criteria.AGGREGATE_SKIP_STAGE.getDescription(),
                Criteria.AGGREGATE_SKIP_STAGE.getRequiredCount()
        );
    }
    ...
    public void visitPipelineParameter(PipelineParameter parameter) {
        if (curParamIdx == 0) {
            for (DocumentParameter documentParameter : parameter.getParameterList()) {
                if (documentParameter.containsField("$skip", 1)) {
                    currentScore++;
                    satisfied = true;
                    return;
                }
